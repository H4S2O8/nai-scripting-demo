...
结果图占掉除控件外的全部高度，点开看大图。下面依次是最近结果的缩略条、提示词
预览（点开进全屏编辑器）、以及一行「两次生成之间会变的东西」：尺寸、张数、
seed 锁定 / 换一个、负面词入口。底部常驻 Anlas 估算和生成键。
...
是在一条三行高的缝里打字，而且底下的 chunk 列表全被挡住。所以点提示词是**全屏
打开**：输入框占满键盘以上的空间，chunk 搜索和标签就在正下方——tag 本来就是从那里
来的。负面词走同一个编辑器。
