...
function PromptRow({
  ...
  return (
    ...
    >
      ...
      <Spacer />
      {action ? (
        // A Button inside a tappable row takes the tap for itself, so the
        // dice can sit on a row that otherwise opens the editor.
      ...
      >
        ...
        <VStack spacing={6} frame={{ maxWidth: "infinity", alignment: "leading" }}>
          ...
          <PromptRow
            ...
            label="特定"
            text={params.prompt}
            placeholder="这一张的动作、构图、场景 · 🎲 词典随机"
            onTap={() => wb.editPrompt({ kind: "specific" })}
            emphasis
            action={{ icon: "dice", onTap: () => wb.openCodex("scene") }}
          />
        </VStack>
