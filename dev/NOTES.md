...
**抽取不放在提示词编辑器里，而是生成页第四个槽。** 原因是条目不是一段文本：
所长色色有 23%（2,738 条）的内容在 `characterPrompts` 里，`tags` 只是底稿
（"1girl, kotatsu"），动作全在 char1/char2；还有一批 base 干脆是空的。这些是分镜，
互相引用（"standing before character1"），属于 V4+ 的 `char_captions`，拍扁进一段
提示词会丢掉谁做什么。所以整条存在 `params.codex`，**生成时**才合并：base 排在三块之后，
char_i 追加到人物页第 i 个槽（用户写的外貌在前、条目的动作在后；空槽只用条目的，不钉位置）。
用户写的三块和角色列表**一个字都不改**，清除就是把这一个字段置空。
