...
v3.0.0 重构标签页时，「存相册 / 分享」的按钮没被搬过去。`saveToPhotos` 还好端端地
导出着，全项目零调用——编译过、检查过、测试过，**功能就是没了**。用户报「好像没法
保存」时我才发现。

`check.py` 现在有一条：导出的函数如果全项目（含 dev 测试）除了定义处再没出现过，
就报错。第一版判据写成「在定义它的文件之外没出现」，把 `isV5`、`loadKeystore` 这些
在自己文件里被调用的全误报了 —— 检查规则本身也要两个方向都测，`dev/test_check.sh`
里现在有对应的用例。

顺带：生成页那个图标本来是 `square.and.arrow.down`（iOS 里是「保存/导出」的意思），
绑的却是"复用参数"。图标撒谎比没有图标更糟。
...
瘦身成 `{id, title, path, tags, characters, identity}`，按 release 缓存；之后每次只拉指针。
指针拉不到时直接用缓存并注明，抽随机这种功能宁可稍旧也别没有。

界面照抄站上的 `#chipRail`：一排大类 chip 单选，选中的有子类就在下面再出一排，
一个按钮从当前范围随机。第一版做成了逐层下钻列表外加 12 本法典切换，用户一眼就说
「不是这个」——功能等价，体感完全不同。参照物就在那，照着做比自己发明省事。

**抽取落在用户已有的块里，作为带标记的 chunk。** 先后试过两种别的：放在提示词编辑器内
（一个块看不到角色部分）、生成页独立第四个槽 + 生成时合并（用户看不到合并结果、也改不了）。
最终形态是 `draws.ts`：画师串 → 艺术风格块，场景 base → 特定块，角色 i → 人物页第 i 个槽，
全部是 `🎨 名` / `🎲 名` 标签，在编辑器里可展开、可改、可删。没有隐藏状态、没有合并步骤，
所见即所发。「换一个」按标签前缀原地替换；抽取自己建出来的空槽在不再需要时随之删掉，
用户碰过的行（有文字、负面词或钉了位置）永远不删。

所长色色有 23%（2,738 条）的内容在 `characterPrompts` 里，`tags` 只是底稿，动作全在
char1/char2，还有一批 base 干脆是空的。这些是分镜，互相引用（"standing before character1"），
属于 V4+ 的 `char_captions`——所以角色部分必须按序号进槽，不能拍扁。

普查过一遍 5,693 条 char prompt：87% 是动作/交互，只有 4% 含发色/瞳色/耳朵这类
会盖过用户角色的强身份词。瘦身时按正则标出来（`identity`），界面上给个 ⚠。
身材词（curvy、small breasts）故意不算——那是姿势词汇的一部分，算进去会标掉半本。

抽过的记录按法典存 id（`nai.codex.drawn.<id>`），抽取池排除已抽的；一类抽干了给
「重置已抽」，只清当前范围，不动别的分类。

选择页每次打开先拿内存里的词典上屏（没有才读磁盘缓存），**再**去后台问网站有没有更新。
第一版是反过来的：先三次网络往返 + 4 MB 解析，期间 chip 全灰——用户报的就是「生成完再打开，
换一个是灰的，要等」。缓存在这里分三层：内存（本次运行）、磁盘（重启后）、网络（只问变没变）。

随机画师走另一条路：画师串只是一段画风标签，用户本来就有「艺术风格」块装它，所以不另开槽，
而是以带前缀（`🎨 `）的 chunk 落进那个块——在编辑器里显示为一个标签、双击展开、可删；
「换一个」按前缀找到上一个原地替换。选择页按法典参数化（`CodexSpec`），画师那本默认停在最新批次，
因为那一批才是刚对着当前模型测过的。同一本里有人写 `artist:name`、有人写裸名，两种都合法，照原样插。

站上的「7.15更新 / 8.31更新」是按**更新批次**筛：`codexes.json` 每本有 `updateFilters`，
条目上有 `updateBatches`（可属多批；批次追踪之前的原有条目为空）。选择页在分类排下面
再加一排批次 chip，计数按**当前分类**算——那才是马上要抽的数。分类和批次是先后两层过滤，
顺序不影响结果。

## 本机词库分两层（v3.12.1）

症状：在 A 账号下新建的 chunk，切到 B 账号看不见；在 A 下拉取一次，它还会直接消失。

原因：词库缓存整个按账号存（`nai.chunks.v1.<id>`），当初是为了 mirror 推送不会带着别的
账号的词库去删东西。这个理由只对「从账号拉下来的」chunk 成立，却把本机新建 / 导入的
也一起锁进了账号。

现在 `chunks.ts` 里分两层：账号层（带 `remoteId`，仍按账号存）和设备层
（`nai.chunks.local.v1`，不带 `remoteId`，所有账号共见）。`loadCache` 合并两层，账号层
同 id 优先；`saveCache`（编辑后）按 `remoteId` 有无拆开存，已被本账号推送过的设备
chunk 会用推送后的内容刷新设备副本（改动才能在别的账号下读到）；`savePulled`
（拉取后）只覆盖账号层。root 分类是排序，永远按账号存，所以设备 chunk 在别的账号
下落在「未分类」里，推送后再被那个账号的 root 收编。删除是「从本机删」：两层都删，
但已经在别的账号服务器上的副本留着——那本来就会在下次拉取时回来，要删得走 mirror。

旧版留在账号层里的未推送 chunk 会在第一次 `loadCache` 时被提到设备层，不用等下次编辑。
